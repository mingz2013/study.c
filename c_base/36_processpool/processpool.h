...
{
private:
	// 将构造函数定义为私有的，因为我们只能通过后面的create静态函数来创建processpool实例
...
private:
	...
	// 子进程在池中的序号，从0开始
	int m_idx;
	// 每个进程都有一个epoll内核事件表，用e_epollfd标识
	int m_epollfd;
	// 监听socket
	int m_listenfd;
...
// 信号处理函数，将接收到的信号写入管道里，在主循环里处理信号事件
static void sig_handler(int sig)
{
	int save_errno = errno;
	int msg = sig;
	send(sig_pipefd[1], (char*)&msg, 1, 0);// 写入 信号管道里
	errno = save_errno;
}
...
processpool<T>::processpool(int listenfd, int process_number)
	:m_listenfd(listenfd), m_process_number(process_number), m_idx(-1), m_stop(false)
	{
		...
		{
			int ret = socketpair(PF_UNIX, SOCK_STREAM, 0, m_sub_process[i].m_pipefd);	// 建立双向管道
			assert(ret == 0);
...
void processpool <T>::setup_sig_pipe()
{
	...
	addfd(m_epollfd, sig_pipefd[0]);	// 信号管道
	...
	while(!m_stop)
	{
		...
		for(int i = 0; i < number; i++)
		{
			...
			if(sockfd == m_listenfd)
			{
				...
				if(m_sub_process[i].m_pid == -1)	// 没有找到可用的进程
				{
					m_stop = true;
					break;
				}
				...
				else
				{
					for(int i = 0; i < ret; ++i)
					{
						switch(signals[i])
						{
							...
							case SIGINT:
							{
								// 如果父进程接收到终止信号，那么久杀死所有子进程，并等待特闷全部结束，当然，通知子进程结束更好的方法是向父、子进程之间的通信管道发送特殊数据，读者不妨自己实现之
