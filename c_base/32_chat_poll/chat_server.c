...
int main(int argc, char * argv[])
{
	...
	// 创建users数组，分配FD_LIMIT个client_data对象，可以预期：每个可能的socket链接
	// 都可以获得一个这样的对象, 并且socket的值可以直接用来索引（作为数组的下标）socket连接对应的client_data对象，
	...
	while(1)
	{
		...
		for(int i = 0; i < user_counter + 1; ++i)
		{
			if((fds[i].fd == listenfd) && (fds[i].revents & POLLIN))
			{
				...
				// 对于新的连接，同时修改fds和users数组，前文已经提到，users[connfd]对应于新链接文件描述符connfd的客户数控
				user_counter++;
